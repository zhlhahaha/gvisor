...
// scaledTSC returns the host TSC scaled by the given frequencies.
//
// This assumes a current frequency of 1. See setSystemTime for context.
//
...
func (c *vCPU) setSystemTime() error {
	...
	if _, _, errno := syscall.RawSyscall(
		...
		_KVM_SET_TSC_KHZ,
		1 /* khz */); errno != 0 {
		return fmt.Errorf("tsc scaling not supported: %v", errno)
	}
	defer func() {
	...
	for {
		...
		if _, _, errno := syscall.RawSyscall(
			...
			return fmt.Errorf("error getting tsc: %v", errno)
		}
