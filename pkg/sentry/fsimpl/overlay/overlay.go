...
func (fstype FilesystemType) GetFilesystem(ctx context.Context, vfsObj *vfs.VirtualFilesystem, creds *auth.Credentials, source string, opts vfs.GetFilesystemOptions) (*vfs.Filesystem, *vfs.Dentry, error) {
	...
	if haveFSOpts {
		...
		// filesystem filesystem with 501 lower layers if desired.
	} else {
	...
	if rootStat.Mode&linux.S_IFMT == linux.S_IFDIR {
		...
		root.ino = atomic.AddUint64(&fs.lastDirIno, 1)
...
func (d *dentry) topLayerInfo() (vfs.VirtualDentry, bool) {
	if d.isCopiedUp() {
...
func (d *dentry) updateAfterSetStatLocked(opts *vfs.SetStatOptions) {
	if opts.Stat.Mask&linux.STATX_MODE != 0 {
		atomic.StoreUint32(&d.mode, (d.mode&linux.S_IFMT)|uint32(opts.Stat.Mode))
	}
