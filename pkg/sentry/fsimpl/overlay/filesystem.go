...
func (fs *filesystem) lookupLocked(ctx context.Context, parent *dentry, name string) (*dentry, error) {
	...
	if child.isDir() {
		...
		child.ino = atomic.AddUint64(&fs.lastDirIno, 1)
...
func (fs *filesystem) lookupLayerLocked(ctx context.Context, parent *dentry, name string) (lookupLayer, error) {
	...
	parent.iterLayers(func(parentVD vfs.VirtualDentry, isUpper bool) bool {
		...
		if stat.Mode&linux.S_IFMT == linux.S_IFCHR && stat.RdevMajor == 0 && stat.RdevMinor == 0 {
			// This is a whiteout, so it "doesn't exist" on this layer, and
			// layers below this one are ignored.
			if isUpper {
...
func (fs *filesystem) doCreateAt(ctx context.Context, rp *vfs.ResolvingPath, dir bool, create func(parent *dentry, name string, haveWhiteout bool) error) error {
	var ds *[]*dentry
	fs.renameMu.RLock()
...
func (fs *filesystem) LinkAt(ctx context.Context, rp *vfs.ResolvingPath, vd vfs.VirtualDentry) error {
	return fs.doCreateAt(ctx, rp, false /* dir */, func(parent *dentry, childName string, haveWhiteout bool) error {
...
func (fs *filesystem) MkdirAt(ctx context.Context, rp *vfs.ResolvingPath, opts vfs.MkdirOptions) error {
	return fs.doCreateAt(ctx, rp, true /* dir */, func(parent *dentry, childName string, haveWhiteout bool) error {
		vfsObj := fs.vfsfs.VirtualFilesystem()
...
func (fs *filesystem) ListxattrAt(ctx context.Context, rp *vfs.ResolvingPath, size uint64) ([]string, error) {
	...
	// Linux overlayfs actually allows listxattr, but not any other xattr
	// syscalls. For now we just reject all of them.
