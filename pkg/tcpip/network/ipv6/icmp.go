...
func getSourceLinkAddr(it header.NDPOptionIterator) (tcpip.LinkAddress, error) {
	var sourceLinkAddr tcpip.LinkAddress
	for {
		opt, done, err := it.Next()
		if err != nil {
			// This option is not valid as per the wire format, silently drop the
			// packet.
			return "", fmt.Errorf("invalid option as per the wire format")
		}
		if done {
			break
		}
		...
		case header.NDPSourceLinkLayerAddressOption:
			// No RFCs define what to do when an NDP message has multiple Source
			...
			if len(sourceLinkAddr) != 0 {
				return "", fmt.Errorf("found multiple source link address options")
			}

			sourceLinkAddr = opt.EthernetAddress()
		}
	}
	return sourceLinkAddr, nil
}
...
func (e *endpoint) handleICMP(r *stack.Route, netHeader buffer.View, pkt stack.PacketBuffer, hasFragmentHeader bool) {
	...
	case header.ICMPv6RouterSolicit:
		...
		// The remainder of payload must be only the router solicitation, so
		// payload.ToView() always returns the solicitation. Per RFC 6980 section
		// 5, NDP messages cannot be fragmented. Also note that in the common case
		// NDP datagrams are very small and ToView() will not incur allocations.
		...
		sourceLinkAddr, err := getSourceLinkAddr(it)
		if err != nil {
			received.Invalid.Increment()
			return
		}
	...
	case header.ICMPv6RouterAdvert:
		...
		if len(sourceLinkAddr) != 0 {
			e.nud.HandleProbe(routerAddr, r.LocalAddress, header.IPv6ProtocolNumber, sourceLinkAddr)
		}
