...
func (e *neighborEntry) addWakerLocked(w *sleep.Waker) {
	...
	if e.mu.wakers == nil {
		e.mu.wakers = make(map[*sleep.Waker]struct{})
	}
	e.mu.wakers[w] = struct{}{}
}
...
func (e *neighborEntry) dispatchAddEventLocked(nextState NeighborState) {
	if nudDisp := e.nic.stack.nudDisp; nudDisp != nil {
		nudDisp.OnNeighborAdded(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, nextState, time.Now())
	}
}
...
func (e *neighborEntry) dispatchChangeEventLocked(nextState NeighborState) {
	if nudDisp := e.nic.stack.nudDisp; nudDisp != nil {
		nudDisp.OnNeighborChanged(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, nextState, time.Now())
	}
}
...
func (e *neighborEntry) dispatchRemoveEventLocked() {
	if nudDisp := e.nic.stack.nudDisp; nudDisp != nil {
		nudDisp.OnNeighborRemoved(e.nic.id, e.mu.neigh.Addr, e.mu.neigh.LinkAddr, e.mu.neigh.State, time.Now())
	}
}
...
func (e *neighborEntry) setStateLocked(next NeighborState) {
	// Stop the previously scheduled action, if there is one. Entries in Unknown,
	// Stale, or Static state do not have scheduled actions and do not need a
	// timer.
	if e.mu.timer != nil {
	...
	case Incomplete:
		...
		sendMulticastProbe = func() {
			...
			retryCounter++
			e.mu.timer = tcpip.NewCancellableTimer(&e.mu, sendMulticastProbe)
	...
	case Reachable:
		e.mu.timer = tcpip.NewCancellableTimer(&e.mu, func() {
	...
	case Probe:
		...
		sendUnicastProbe = func() {
			...
			e.mu.timer = tcpip.NewCancellableTimer(&e.mu, sendUnicastProbe)
			e.mu.timer.Reset(config.RetransmitTimer)
		}
