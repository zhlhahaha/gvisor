...
TEST(SpliceTest, TwoPipesCircular) {
  // FIXME(gvisor.dev/issue/1624): This test deadlocks the sentry on VFS1
  // because VFS1 splice ordering is based on fs.File.UniqueID, which does not
  // prevent circular ordering between e.g. inode-level locks taken by
  // fs.FileOperations. VFS2's splice is not susceptible to this issue, so this
  // test should be enabled on gVisor once VFS1 is dead.
